...
if __name__ == '__main__':
    ...
    if args.system_type == 'lora_LSTM':
        ...
        enc_model_fpath = Path("{}/{}-{}_train-part_a/encoder_315000.bak".format(model_dir_1, args.dataset_m, args.model))
    ...
    print(args.dataset, args.dataset == 'nus_cms_48')
    if args.dataset == 'nus_cms_48':
        src_root = './storage/NUS-CMS-48'
        backtrack_flag = '_backtrack' if args.backtrack else ''
        des_root = './storage/NUS-CMS-48-adver-full-two-loss{}-psy_scale{}-no-eps'.format(system_flag, backtrack_flag)
        if args.src_flag is not None:
            src_root = f'./storage/NUS-CMS-48-{args.src_flag}'
        if args.des_flag is not None:
            des_root = f'./storage/NUS-CMS-48-{args.des_flag}'
    ...
    for idx_spk, flag in enumerate(spks_keys[args.start:args.end]):
        ...
        try:
            ...
            print('*' * 50, f'using {args.voice_num} voices', '*' * 50)
            ...
            for idx, wav in enumerate(wavs):
                ...
                loss_avg = torch.zeros(3, device=device, dtype=wavs[0].dtype)
                loss_var = torch.zeros(3, device=device, dtype=wavs[0].dtype)
                ...
                for cur_iter in range(N):
                    ...
                    all_loss = torch.zeros(3, device=device, dtype=wavs[0].dtype)
                    ...
                    print('*', flag, idx, cur_iter, loss1.item(), loss2.item(), loss_th.item(), loss_tol.item())
