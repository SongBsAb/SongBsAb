...
from whisper_so_vits_svc_5.model import Whisper, ModelDimensions
from whisper_so_vits_svc_5.audio import load_audio, pad_or_trim, log_mel_spectrogram
...
def pred_ppg_t_batch(whisper: Whisper, audio):

    # for p in whisper.parameters():
    #     print('s3l test 1:', p.dtype, p.device, whisper.device)
    ...
    if (idx_s < audln):
        ...
        # short = pad_or_trim(short)
        mel = log_mel_spectrogram(short).to(whisper.device)
        # print('s3l test:', mel.dtype, mel.device)
        if not (whisper.device == "cpu"):
            mel = mel.half()
        # print('s3l test:', mel.dtype, mel.device)
        mel = mel + torch.randn_like(mel) * 0.1
